...
export class Robot {
  ...
  _replan(): void {
    if (this.goal !== null) { this._planTo(this.goal); this.stats.replans++; }
  }
  ...
  publishState(tick: number): void {
    ...
    if (!this._shouldBroadcast(peerStates)) return; // v2: skip if not needed

    const nextNode = this.path.length > 1 ? this.path[1] : this.pos;
    this.bus.publishState(this.id, {
      tick, id: this.id, pos: this.pos, goal: this.goal,
      intent_next: nextNode, eta: this.path.length - 1,
      priority: this.priority, task_id: this.task_id,
      battery: Math.round(this.battery * 10) / 10, state: this.state,
    });
  ...
  step(tick: number, occupancy: Map<string, number>): StepResult {
    ...
    if (this.path.length < 2) {
      this._replan();
      if (this.path.length < 2) {
        this.state = 'waiting'; this.waiting_on = null;
    ...
    if (this._localAvoid.has(Warehouse.edgeKey(this.pos, nextNode)) || this.wh.edgeBlocked(this.pos, nextNode)) {
      this._replan();
      if (this.path.length < 2) { this.state = 'waiting'; return { id: this.id, event: 'blocked_local', pos: this.pos }; }
    ...
    this.pos = nextNode;
    this.path.shift();
    this.state = 'moving'; this.stats.ticks_moving++;
  ...
  getState(): 'idle' | 'moving' | 'waiting' | 'failed' { return this.active ? this.state : 'failed'; }
}
